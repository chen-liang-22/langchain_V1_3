...
# 5. 绑定结构化输出
model_with_structured_output = llm.with_structured_output(Movie, method="json_mode")
...
for actor in resp.actors:
    print(
        f"  真实姓名: {actor.real_name} | 角色名: {actor.role_name} | 性别: {actor.gender} | 拍摄时年龄: {actor.age_at_filming}")
